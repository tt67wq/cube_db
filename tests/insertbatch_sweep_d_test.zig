//! insertbatch_sweep_a_test.zig — T-54-C shard d (of a..d) of the T-42
...
test "T-42: branch-producer error path — no UAF, no leaks (calibrated fault sweep, shard d)" {
    const total = try h.countAllocs(h.branchOverflowScenario);
    const r = h.shardRange(total, 3);
    try h.sweepFailIndexes("branch-d", h.branchOverflowScenario, r.first, r.last_exclusive);
    diag.print("SWEEP shard=d total={d} first={d} last={d} points={d}\n", .{ total, r.first, r.last_exclusive, r.last_exclusive - r.first });
}
