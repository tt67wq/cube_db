...
test "T-42: branch-producer error path — no UAF, no leaks (calibrated fault sweep, shard c)" {
    const total = try h.countAllocs(h.branchOverflowScenario);
    const r = h.shardRange(total, 2);
    try h.sweepFailIndexes("branch-c", h.branchOverflowScenario, r.first, r.last_exclusive);
    diag.print("SWEEP shard=c total={d} first={d} last={d} points={d}\n", .{ total, r.first, r.last_exclusive, r.last_exclusive - r.first });
}
