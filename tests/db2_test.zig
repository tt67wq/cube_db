//! db2_test.zig — Db2 集成测试（TDD RED）
//! 覆盖：open/close、put/get、putBatch、delete、select、meta 恢复。
//! 用 MemPageStore 模拟持久化。先 fail（db2.zig 不存在）。
...
test "db2: open with default state" {
    var ms = newStore();
    defer ms.deinit();
...
test "db2: put and get roundtrip" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "db2: get missing key returns null" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "db2: putBatch then get all" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
    const entries = [_]db2.Entry{
        .{ .key = "a", .value = "1" },
        .{ .key = "b", .value = "2" },
        .{ .key = "c", .value = "3" },
    };
...
test "db2: 100 sequential puts all readable" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "db2: select full range ordered" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "db2: close and reopen recovers from meta" {
    var ms = newStore();
    defer ms.deinit();
    const s = ms.store();

    // 第一次打开，写入数据
    var db = try db2.Db2.open(std.testing.allocator, s, .{});
    ...
    var db2_ = try db2.Db2.open(std.testing.allocator, s, .{});
    defer db2_.close();
...
test "db2: meta alternation — write, corrupt one meta, recover" {
    var ms = newStore();
    defer ms.deinit();
    const s = ms.store();
