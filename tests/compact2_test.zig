//! compact2_test.zig — compact v2 测试（TDD RED）
...
test "compact2: compact on empty db is no-op" {
    var ms = newStore();
    defer ms.deinit();
...
test "compact2: compact clears dirt after writes" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "compact2: compact preserves data" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "compact2: compact with active reader blocks" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "compact2: multiple compacts are idempotent" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
...
test "compact2: after compact, new writes work" {
    var ms = newStore();
    defer ms.deinit();
    var db = try db2.Db2.open(std.testing.allocator, ms.store(), .{});
    defer db.close();
