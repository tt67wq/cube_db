...
### 4.5 报告测试总数：必须「冷缓存 + `--summary all` + 绿跑」

**T-38-4 期间 conductor 被这个机制误导过一次**：把默认门总数测成 **539**，据此怀疑
「本任务新增的 11 个用例里有 4 个没进默认门」（即 T-54 主线在查的「测试静默不执行」类别）。
真相不是缺测试，是**计数口径**：

- `zig build test` 的汇总行 `N/M tests passed` **只统计本次调用里实际执行的 run-test step**；
  **缓存命中的 run step 计 0**。
- 于是「坏 seed 导致失败 + 其余 run step 全部缓存命中」的运行会报出一个**偏小的假总数**
  （实测 `538/539`，而冷缓存绿跑是 `543/543`）。
- `-Dfilter=` 的 `test-one` 不受此影响（它只跑匹配的文件）。

**约定**：任何对外引用/写进台账的测试总数，必须满足三条 ——
**① 冷缓存**（或注明缓存状态）；**② `--summary all`**；**③ 绿跑**（失败运行的计数会被截断）。
只写总数不写口径的场合，宁可写「exit 0 且无失败」（见 §4.1）。
