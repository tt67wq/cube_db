...
> 最后更新：2026-09-20（**T-54 全阶段完成并 closed 归档**；新立 T-55 收尾 nit；**修正 T-38 过期表述 —— 阶段 3 已合入 `fe2f575`，只剩阶段 4 GC**）。main = `93259d4`。
...
`**CLOSED** ✅`（N-1）与 `**closed**`（T-51）应读作 `closed`（已归档文件不回改）。
...
|---|---|---|---|---|
| **T-38** | deleteRange 高效化：全量物化 + 每 key tombstone 的 O(range) 内存与写放大 | `fixing` | **阶段 1/2/3 已合入验收**（阶段 3 写路径 = `fe2f575`：区间墓碑链 + 打洞/分裂 + version 2→3 + entryCount 流式）。**只剩阶段 4（GC）**：水位收割（无并发读者 + 区间内无在场 key → 墓碑直接丢弃；探路报告 §6 称「实现小、收益大」）+ crash 矩阵扩展；**物化清除挂 U-5 真·compact**。前置 T-49/T-50/T-52 全关闭 | `baa44bd`、`88124bc`、`fe2f575` |
...
- **T-38** 是唯一的 `fixing`，是本仓库当前的主线工作。阶段 3（写路径）已合入 `fe2f575`，
  前置 T-49/T-50 已由 `1c6ce1d` 关闭、T-52 已修复归档 → **阶段 4（GC）现可推进**。
