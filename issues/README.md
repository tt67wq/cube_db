...
> 最后更新：2026-09-18（新立 T-54：测试效率长尾 + 49 个孤儿测试）。main = `c09666f`。
...
| **T-54** | 测试效率：单个 180s step 独占 wall time + 49 个测试从不执行 | `open` | medium-high，三条痛点（效率 / 漏测面 / 17GB 内存炸弹）。分 P0–P4；**P0 必须先做**（零风险量准 180s 构成），P1 预期 wall 182s → 50–60s | — |
...
- **T-54** 是新增的 `open`，属于**工程效率 / 漏测**面（不是数据正确性），但影响每一次迭代：
  182s 里 180s 由单个 step 独占，且 **49 个已写好的测试从未进过 CI**（含整个 fuzz 套件）。
  建议先做 P0（零风险量测），再决定 P1 的拆法。
