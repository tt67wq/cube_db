...
> 最后更新：2026-09-18（T-54 转 `partial`：P0 量准 + P4.1/P4.2 已合入验收）。main = `4a27c22`。
...
| **T-54** | 测试效率：单个 180s step 独占 wall time + 49 个测试从不执行 | `partial` | **P0（量准）+ P4.1/P4.2（17GB 内存炸弹 + 日志误导）已合入验收**。还剩 **P1（拆长尾，wall 182s → ~50s，主收益；方案已定为 T4 4 路分片）**、P2（迭代入口 <5s）、P3（49 个孤儿测试接进验收 + `build.zig` 844→<150 行）、P4.3（README）。下一步 = 派 T-54-C（P1 实施） | `b43dcd6`、`4a27c22` |
...
- **T-54** 已从 `open` 转 `partial`：P0 量准（180s 是**单个测试** T4 fault sweep 独占，不是多测累加）
  与 P4（17G→1G、诊断默认静默）已合入。**剩下的 P1 才是主收益**（预期 wall 182s → ~50s），
  方案已定为「T4 4 路分片并行」（`0d011cb`）。P1 未落地前，每次全量测试仍是 **3 分钟**。
  且 **49 个已写好的测试仍未进过 CI**（含整个 fuzz 套件）。
