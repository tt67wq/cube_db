...
- [x] 更正设计文档 §2 表格行 2（「打开失败」→「静默按空库打开并覆盖数据」）
      + 写明真实部署风险；由 T-53 一并回写。
- [x] 开库路径对未识别版本的**显式拒绝**已在 T-53 落地
      （`readMeta()` 的 `null` 语义细分：未初始化 vs 不可识别 → `error.InvalidMeta`）。
- [x] 验收通过后置为 closed。
...
- `zig build test-openmeta` → 9/9，exit 0（a4 直接覆盖本 issue 的 FPS 证据链：
  v4 库被 `Db.open` **拒绝**，且**拒绝前后文件逐字节相同**）；
- 独立对抗用例 adv1：v2 库真实数据页 + 双槽改 v4/v5/v65535/v0/v1 + 坏 magic
  → **全部拒绝**、**零字节改动**、改回原 meta 后**数据完好**；
- 回归门 `test-rangetomb-read` 15/15、`test-tombguard` 2/2、全量 exit 0 无失败。
