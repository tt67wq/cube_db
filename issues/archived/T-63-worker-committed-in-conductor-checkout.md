...
- **状态**: closed（T-61-1 模板化收口：worktree 硬约束行进 task.md.example、conductor 断言进 README SOP、T-58/T-64R2 起两 worker 零复发实证）（**2026-09-23 复发**：T-53-1 轮 pi1 再次把分支 checkout+commit 落在主 checkout，conductor 的 `git merge`/gate 差点再踩一次；建议措施优先级升为「契约模板必含硬约束行 + conductor merge 前机械断言」，两者均已进 T-61-1 交付物）
