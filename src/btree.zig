...
fn readRecord(allocator: std.mem.Allocator, s: Store, offset: u64) ![]u8 {
    // 先读 len(4)，再读完整记录。
...
fn decodeNodePayload(rec: []const u8) ![]const u8 {
...
// ===== 写入 store 辅助 =====
fn appendLeaf(s: Store, allocator: std.mem.Allocator, leaf: *const Leaf) !u64 {
    const rec = try leaf.toRecord(allocator);
    defer allocator.free(rec);
    return s.append(rec);
}

fn appendBranch(s: Store, allocator: std.mem.Allocator, branch: *const Branch) !u64 {
...
// 节点缓存 + 脏集 + 一次 flush（自底向上）。apply 写缓存节点，commit 一次写所有脏节点。
// 所有内存（解码节点、dup key/value、CachedNode、临时数组）由 arena 管理，deinit 整体 free。
