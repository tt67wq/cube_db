...
2. **skip-decode**：热路径跳过反序列化，一个字节游标从头顺序扫描原始 payload，靠 key 有序性提前退出——不建结构数组、不拷贝（注：变长 entry 无偏移表，没法 O(1) 跳转，所以是顺序扫而非二分；省的是 alloc+拷贝，不是比较次数）
...
> **页地址式 B 树 vs 指针式**：v1 节点持 **u64 字节偏移**（物理地址，绑死存储字节布局）；v2 节点只持 **页号（u32）**（逻辑地址），要用时才从 page store 取。看似多一层间接，但它让 B 树**与具体存储解耦**——能放内存、能放文件、能放 mmap。这是 COW 能成立的前提。
