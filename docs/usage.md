...
### 3.3b 空 key 与墓碑端点语义（T-57）

空 key（空串 `""`）是**合法** key：`put("", v)` / `get("")` / `delete("")` 均正常工作（空串是最小的 key）。

在存在区间墓碑（`deleteRange` 产生）的库上，墓碑端点的空串语义如下（T-57 修复后的保证）：

- 作为 **min** 端点：空串与「无下界（null）」在磁盘编码上等价 —— 空串是最小 key，`k >= ""` 覆盖全部 key，两种写法的覆盖范围相同；
- 作为 **max** 端点：`[x, "")` 是**空区间**（不存在小于空串的 key），内部会在产出时丢弃该段，不会发布。

**旧库注意（T-57 修复之前写入的数据）**：修复前，给空 key 打洞留下的、本意为 `[min, "")`
的墓碑段，其磁盘编码与真正的 `[min, +inf)` 全区间墓碑**逐字节相同**（端点「空串」与「无界」共用
同一编码），无法在不改磁盘格式的前提下区分。因此这类旧库可能仍处于「全部 key 被遮蔽」的状态。
修复只保证**新写入**的墓碑段干净，不会自动修复旧数据。

自查方法：若发现 `select(null, null)` / `get(k)` 看不到任何 key，而 `entryCount()` 非 0
（两个口径背离）→ 该库受影响，需要**重建**（导出可见数据重建新库）或人工修复墓碑链；
仅靠继续写入无法自愈。
